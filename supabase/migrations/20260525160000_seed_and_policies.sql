...
-- 2. Ensure RLS Policies for all necessary tables
...
CREATE POLICY "Users can manage own messages" ON public.messages FOR ALL TO authenticated USING (
  EXISTS (SELECT 1 FROM leads l WHERE l.id = messages.lead_id AND l.user_id = auth.uid())
) WITH CHECK (
  EXISTS (SELECT 1 FROM leads l WHERE l.id = messages.lead_id AND l.user_id = auth.uid())
);
