...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM auth.users WHERE email = 'geamefialho@hotmail.com') THEN
    ...
    INSERT INTO public.whatsapp_configs (user_id, phone_number_id, access_token, verify_token, connection_type)
    VALUES (new_user_id, '', '', 'meu_token_secreto_123', 'official')
    ON CONFLICT (user_id) DO NOTHING;
  END IF;
END $$;
